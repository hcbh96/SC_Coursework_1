...
def test_return_continuous_solution_to_hopf_bifurcation_using_newton_and_solve_ivp():
    ...
    # act
    sol=npc(func_wrapper, X0, vary_par, method='shooting', boundary_vars=b_vars, integrator=solve_ivp, root_finder=newton)
    # assert
    print(sol)
    assert np.allclose(sol[0], [16.779, -4.695], atol=1e-02)
    assert np.allclose(sol[-1], [0,1], atol=1e-02)
...
def test_return_continuous_solution_to_hopf_bifurcation_using_newton_and_odeint():
    ...
    # act
    sol=npc(func_wrapper, X0, vary_par, method='shooting', boundary_vars=b_vars, root_finder=newton, integrator=odeint)
    # assert
    print(sol)
    assert np.allclose(sol[0], [5109, 2398], rtol=1e-02)
...
def test_return_continuous_solution_to_hopf_bifurcation_using_fsolve_and_solve_ivp():
    ...
    # act
    sol=npc(func_wrapper, X0, vary_par, method='shooting', boundary_vars=b_vars, root_finder=fsolve,integrator=solve_ivp)
    # assert
    assert np.allclose(sol[0], [1, 1], atol=1e-02)
    assert np.allclose(sol[-1], [15.857,-0.156], atol=1e-02)
    assert len(sol) == 10, "Should be 10"
...
def test_return_continuous_solution_to_hopf_bifurcation_using_fsolve_and_odeint():
    ...
    # act
    sol=npc(func_wrapper, X0, vary_par, method='shooting', boundary_vars=b_vars, root_finder=fsolve, integrator=odeint)
    # assert
    assert np.allclose(sol[0], [1, 1], atol=1e-02)
